...
// const q2 = query(teamsRef, where("country", "==", "Spain"), where("city", "==", "Madrid"));
...
// snapshot4.docs.filter(doc => doc.data().country !== "Spain").forEach(doc => console.log("Not Spain:", doc.data()));

// 5. Show all teams not in Spain or England
// const snapshot5 = await getDocs(teamsRef);
// snapshot5.docs.filter(doc => !["Spain", "England"].includes(doc.data().country))
...
// snapshot6.docs.filter(doc => doc.data().country === "Spain" && doc.data().worldwideFans > 700)
//   .forEach(doc => console.log("Spain + >700M fans:", doc.data()));

// 7. Teams with 500M–600M fans
// const q7 = query(teamsRef, where("worldwideFans", ">=", 500), where("worldwideFans", "<=", 600));
